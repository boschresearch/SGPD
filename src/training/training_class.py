...
class Training:
    ...
    def train(
        self, dataset: DataLoader, n: int, frac: float, x0, val: torch.Tensor, steps: int, ref_data: torch.Tensor, mode:str
        ...
        Args:
            ...
            x0 (horizon, d)-torch.Tensor): initial value.
            test ((batchsize, d, N)-torch.Tensor): test data.
            ref_data ((batchsize, d, N)-torch.Tensor): ground truth data
        Returns:
            ...
            epoch_loss (torch.Tensor): training loss over epochs.
            model (torch.Tensor): result from model selection. 
        """

        frac, loss_matrix, epoch_loss, val_loss, ref_loss, model_list, test_steps, test_batch = self.initialize_matrices(n, val)
        ...
        for c in range(0, self.trainingSteps):
            ...
            if val is not None:
                      ...
                      for i in range(5):
                          self.sample_model("sample_predictions")                  
                      ...
                      loss = torch.sqrt(
                          torch.sum((val[:, 1:, :] - X_mean[:, 1 : val.shape[1], :]) ** 2) / (val.shape[1] - 1)
                        )
        ...
        print("finished")
        index = torch.argmin(torch.Tensor(val_loss[105:]))
        model = model_list[index + 105]
        return self.p, self.q, loss_matrix, val_loss, epoch_loss, model, index
